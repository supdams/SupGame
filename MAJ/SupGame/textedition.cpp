...
int TE_NewTextEdition(TextEdition *te, int length, SDL_Rect pos, TTF_Font *font, SDL_Color colorFG, int style)
{
    ...
    te->text = (char *)malloc(sizeof(char)*(length+1));
    if (!te->text)
        return 0;
    te->text[0] = '\0';
    te->pos = pos;
    ...
    te->tab = (TE_CharInfo**)malloc(sizeof(TE_CharInfo*));
    te->tab[0] = (TE_CharInfo*)malloc(sizeof(TE_CharInfo));
    ...
    switch (event.key.keysym.sym)
    {
        ...
        case 'c':
            if (te->selection.begin != te->selection.end)
            {
                if (TE_clipBoard)
                    free(TE_clipBoard);
                if ( (TE_clipBoard = (char*)malloc(sizeof(char)*(te->selection.end-te->selection.begin+1))) )
                {
...
static int NewLetter(TE_CharInfo **tab, int l, int c)
{
    ...
    for (; tab[l][i].c ; i++);
    ptr = (TE_CharInfo*)malloc(sizeof(TE_CharInfo)*(i+2));
    if (!ptr)
        return 0;
...
static int NewLine(TextEdition *te, int l)
{
    ...
    tab = (TE_CharInfo**)malloc(sizeof(TE_CharInfo*)*(++ te->numLines));
    if (!tab)
        return 0;

    for(i = te->numLines-1 ; i>l ; i--)
        tab[i] = te->tab[i-1];
    tab[i] = (TE_CharInfo*)malloc(sizeof(TE_CharInfo));
    if (!tab[i])
    {
        free(tab);
        return 0;
...
static int InsertChar(char text[], unsigned int pos, char c, unsigned int lengthMax)
{
    ...
    if (l+1 >= lengthMax || pos > l)
        return 0;
    if ( !(buf=(char*)malloc(sizeof(char)*(lengthMax+1))) )
        return 0;
...
int TE_SetEditionText(TextEdition *te, const char text[])
{
    ...
    if (!te || !text || !(buf = (char*)malloc(sizeof(char)*(strlen(text)+1))) )
        return -1;
